<?php
	...
	function listIdeas() {
		...
		$stmt = $mysqli->prepare("SELECT vp_userideas.id, vp_userideas.idea, vp_userideas.ideacolor, vp_userideas.userid, vp_users.firstname, vp_users.lastname FROM vp_userideas INNER JOIN vp_users on vp_userideas.userid = vp_users.id AND vp_userideas.deleted IS NULL ORDER BY vp_userideas.id DESC");
		...
		$stmt->bind_result($idDb, $ideaDb, $colorDb, $userID, $firstNameDb, $lastNameDb);
		$stmt->execute();
		while($stmt->fetch()){ 
			$colorinv = color_inverse($colorDb);
			$notice .= '<p style="color:'.$colorinv .';background-color:'.$colorDb .';">'.$ideaDb .'<i> -' .$firstNameDb .' '.$lastNameDb .'</i> | <a href="edituserideas.php?id='.$idDb .'">Toimeta</a></p>'; 
			//echo $colorDb;
			//echo $colorinv;
		}
		$stmt->close();
